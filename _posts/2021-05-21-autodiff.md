...
\begin{align}
y &= e^{2 x_1} + x_1 x_2^2 + \cos x_2 \\\
&= e^{2 w_1} + w_1 w_2^2 + \cos w_2 & \text{(Encode data)} \\\
&= e^{w_3} + w_1 w_4 + w_5 & \text{(Multiply)} \\\
&= w_6 + w_1 w_4 + w_5 & \text{(Exponentiate)} \\\
&= w_6 + w_7 + w_5 & \text{(Multiply)} \\\
&= w_8 + w_5 & \text{(Add)} \\\
&= w_9. & \text{(Add)} 
\end{align}
...
\dot{w_1} &= 1 \\\
\dot{w_2} &= 0 \\\
\dot{w_3} &= \frac{\partial w_3}{\partial w_1} \cdot \dot{w_1} \\\
\dot{w_4} &= \frac{\partial w_4}{\partial w_2} \cdot \dot{w_2} \\\
\dot{w_5} &= \frac{\partial w_5}{\partial w_2} \cdot \dot{w_2} \\\
\dot{w_6} &= \dot{w_1} w_4 + w_1 \dot{w_4} \\\
\dot{w_7} &= \dot{w_3} + \dot{w_6} + \dot{w_5}.
\end{align}
