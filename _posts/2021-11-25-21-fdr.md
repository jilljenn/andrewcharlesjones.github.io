...
\begin{align}
p_{\theta_0}\left(\bigcup_{m=1}^M \text{Reject $H_0^m$}\right) &\leq \sum\limits_{m=1}^M p_{\theta_0}(\text{Reject $H_0^m$}) & \text{(Union bound)} \\\
&= \sum\limits_{m=1}^M \alpha / M \\\
&= \alpha.
\end{align}
...
\begin{align}
FDR(t) &\approx \frac{ \sum\limits_{m=1}^M \mathbb{E}\left[ 1\\{ P_m \leq t \\} | H_m = 0 \right] p(H_m = 0)}{\sum\limits_{m=1}^M \mathbb{E} \left[ 1\\{P_m \leq t\\} \right]} \\\
&= \frac{M \pi_0 t}{M G(t)} \\\
&= \frac{\pi_0 t}{G(t)}
\end{align}
...
\begin{align}
\max \left\\{k : P_k \leq \frac{k}{M} \alpha\right\\} &= \max \left\\{k : P_k \leq \widehat{G}(P_k) \alpha\right\\} \\\
&= \max \left\\{k : \frac{P_k}{\widehat{G}(P_k)} \leq \alpha\right\\}. \\\
\end{align}
...
\begin{align}
p(H = 0 | T \geq t) &= \frac{p(T \geq t | H = 0) p(H = 0)}{p(T \geq t)} \\\
&= \frac{p(T \geq t | H = 0) \pi_0}{p(T \geq t | H = 0) \pi_0 + p(T \geq t | H = 1) \pi_1}.
\end{align}
